    ...
    if ~isempty(tline),
        ...
        if ~isempty(tline)
            ...
            if narg < 7
                ...
                restab(resnum).adr1 = sprintf('%s%i.%s',adstub1,r1,cleanup_Cyana_atom(char(args(3))));
                ...
                restab(resnum).adr2 = sprintf('%s%i.%s',adstub2,r2,cleanup_Cyana_atom(char(args(6))));
            end
        end
    end
end
...
function atag = cleanup_Cyana_atom(atag)
% Cyana uses a double quote instead of two single quotes (PDB standard) for
% some atom names, this must be fixed

poi =strfind(atag,'"');
if ~isempty(poi)
    atag = [atag(1:poi-1) '''' atag(poi+1:end)];
end
