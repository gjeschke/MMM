...
% Version history
% ===============
% Aug 12   MB      > Additional commenting of existing code
%                  > Split the main function into sub-functions for ease of
%                   viewing/maintainence
%                  > Change of export format, so that EVERY line is 80
%                   characters for greater compatibility with other
%                   programs. Achieved using a sprintf command to "output"
%                   rather than incremental file writes with fprintf.
%
...
% Generate header
output              = wr_header(output,model,idCode,snum);

% Title
output              = wr_title(output,model,snum);

% Remarks
[output,numRemark]  = wr_remarks(output,model,modnum,snum,idCode,numRemark);

% Cleaned argument
if ~cleaned
    [output,numSeq,numHet] = wr_cleaned(output,model,snum,cnumbers,...
                        numSeq,numHet);
end

% Helices
[output,numHelix]   = wr_helices(output,model,snum,cnumbers,numHelix);

% Strand
[output,numSheet]   = wr_strand (output,model,snum,cnumbers,numSheet);

% CRYST1
if isfield(model.info{snum},'cryst')
    [output]        = wr_cryst  (output,model,snum);
end

% Atom records
[output,numCoord,numTer,atom_table]  = wr_atom(output,model,chemistry,...
                        modnum,snum,cnumbers,cleaned,numCoord,numTer);

% Connect
[output,numConect] = wr_connect(output,model,snum,cnumbers,atom_table,numConect);

% Master
output = wr_master(output,numRemark,numHet,numHelix,numSheet,...
...
% Open the file for writing, this has been deliberately moved to the end,
% for testing. This way if there is an error in the strucutre handling then
% the file isnt left open in memory. Once testing is complete it could be
% moved to the start of the function again, so that if the file cant be
% written then the error is presented before computation time is wasted.
fid=fopen(fname,'wt');
if fid==-1,
    message.error=1;
...
try
    ...
    % If file writing fails, then close the file (for memory), then delete
    % the file
    fclose(fid);
    delete(fname);
...
function output = wr_header(output,model,idCode,snum)

% write TITLE lines

% 1st line, title, creation date and PDB code
...
header = sprintf('%-80s',header);

output = [output ; header];



function output = wr_title(output,model,snum)

% write HEADER section

tl = size(model.info{snum}.title,1);

for k=1:tl, % loop over all title lines
    if k==1,
        title   = sprintf('TITLE     %s',model.info{snum}.title(k,:));
        title   = sprintf('%-80s',title);
        output  = [output ; title];
        
    else
        title   = sprintf('TITLE   %2i%s',k,model.info{snum}.title(k,:));
        title   = sprintf('%-80s',title);
        output  = [output ; title];
    end
end
...
function [output,numRemark] = wr_remarks(output,model,modnum,snum,idCode,numRemark)

% write REMARK section
...
if isfield(model.info{snum},'Modeller_GA341'),
    ...
    remark6 = sprintf('REMARK   6');
    remark6 = sprintf('%-80s',remark6);
    output  = [output ; remark6];
    
    for k=1:length(GA341),
        remark6 = sprintf('REMARK   6 GA341 OF MODEL %i: %6.4f',k,GA341(k));
        remark6 = sprintf('%-80s',remark6);
        output  = [output ; remark6];
    end;
end;
...
if isfield(model.info{snum},'Modeller_DOPE'),
    ...
    % if
    %   nargin>2 && modnum<=length(model.info{snum}.Modeller_DOPE)&& modnum>=1,
    if  modnum>=1,
    ...
    remark7 = sprintf('REMARK   7');
    remark7 = sprintf('%-80s',remark7);
    output  = [output ; remark7];
    
    for k=1:length(DOPE),
        remark7 = sprintf('REMARK   7 DOPE OF MODEL %i: %10.1f',k,DOPE(k));
        remark7 = sprintf('%-80s',remark7);
        output  = [output ; remark7];
    end;
end;
...
function [output,numSeq,numHet] = wr_cleaned(output,model,snum,cnumbers,numSeq,numHet)
...
for cnum=cnumbers,
    ...
    while poi<residues,
        ...
        seqres = sprintf('%s',tline);
        seqres = sprintf('%-80s',seqres);
        output  = [output ; seqres];
...
for cnum=cnumbers,
    ...
    if isfield(model.structures{snum}(cnum),'mutations'),
        ...
        for k=1:mutated,
            modres = sprintf('MODRES %s %s %s %4i  %s  %s  ',...
                idCode,model.structures{snum}(cnum).mutations(k).modified,...
                cid,model.structures{snum}(cnum).mutations(k).number,...
                model.structures{snum}(cnum).mutations(k).original);
            
            if isfield(model.structures{snum}(cnum).mutations(k),'comment'),
                modres = [modres sprintf('%s',...
                    model.structures{snum}(cnum).mutations(k).comment)];
            end;
            
            output  = [output ; modres];
...
for cnum=cnumbers,
    ...
    if isfield(model.structures{snum}(cnum),'het'),
        ...
        for k=1:hetrecs,
            ...
            het = sprintf('HET    %3s  %s%4i   %5i   ',...
                model.structures{snum}(cnum).het(k).id, ...
                cid, ...
                model.structures{snum}(cnum).het(k).number, ...
                model.structures{snum}(cnum).het(k).hetrecs);
            
            if isfield(model.structures{snum}(cnum).het(k),'descriptor'),
                het = [het sprintf(fid,'%s',model.structures{snum}(cnum).het(k).descriptor)];
            end;
...
function [output,numHelix] = wr_helices(output,model,snum,cnumbers,numHelix)
...
for cnum=cnumbers,
    ...
    if isfield(model.structures{snum}(cnum),'helix_defs') % user-defined helix records
        ...
        for k=1:hnum,
            ...
            helix = sprintf('HELIX  %3i %3s ',helices,htag);
            numHelix=numHelix+1;
            ...
            if ~isempty(end_id),
                ...
                helix = [helix sprintf('%3s %s %4i  %3s %s %4i ',...
                ...
                helix = [helix sprintf('%2i %s %5i',...
                    hclass,comment,end_res-start_res+1)];
                
                helix = sprintf('%-80s',helix);
                output  = [output ; helix];
            end;
        end;
...
function [output,numSheet] = wr_strand(output,model,snum,cnumbers,numSheet)
...
for cnum=cnumbers,
    ...
    if isfield(model.structures{snum}(cnum),'sheet_defs') % user-defined helix records
        ...
        for k=1:shnum,
            ...
            sheet = sprintf('SHEET  %3i %3s 1 ',strands,stag);
            numSheet=numSheet+1;
            ...
            sheet = [sheet sprintf('%3s %s%4i  %3s %s%4i  0',...
                start_name,cid,start_res,end_name,cid,end_res)];
                
            sheet = sprintf('%-80s',sheet);
            output  = [output ; sheet];
        end;
...
if isfield(model.info{snum},'SSbonds'),
    if ~isempty(model.info{snum}.SSbonds),
        for k=1:length(model.info{snum}.SSbonds),
            ...
            SSBONDS = sprintf(...
                'SSBOND %3i CYS %s %4i%s   CYS %s %4i%s                       %s %s %5.2f',...
                k,bond.chainID1,bond.res1,bond.icode1,bond.chainID2,...
                bond.res2,bond.icode2,bond.symop1,bond.symop2,...
                bond.distance);
            
            SSBONDS  = sprintf('%-80s',SSBONDS);
            output = [output ; SSBONDS];
            
        end;
    end;
end;

function output = wr_cryst(output,model,snum)

% write CRYST1 record, if unit cell information is present

cryst=model.info{snum}.cryst;

cryst1 = sprintf('CRYST1%9.3f%9.3f%9.3f%7.2f%7.2f%7.2f %s%4i',...
    cryst.a,cryst.b,cryst.c,...
    cryst.alpha,cryst.beta,cryst.gamma,...
    cryst.sGroup,cryst.Z);
cryst1 = sprintf('%-80s',cryst1);
output = [output ; cryst1];

if isfield(model.info{snum}.cryst,'origx1'),
    origx=model.info{snum}.cryst.origx1;
    cryst1 = sprintf('ORIGX1    %10.6f%10.6f%10.6f     %10.5f',...
...
if isfield(model.info{snum}.cryst,'origx2'),
    ...
    cryst1 = sprintf('ORIGX2    %10.6f%10.6f%10.6f     %10.5f',...
        origx(1),origx(2),origx(3),origx(4));
    cryst1 = sprintf('%-80s',cryst1);
    output = [output ; cryst1];
end;
if isfield(model.info{snum}.cryst,'origx3'),
    ...
    cryst1 = sprintf('ORIGX3    %10.6f%10.6f%10.6f     %10.5f',...
        origx(1),origx(2),origx(3),origx(4));
    cryst1 = sprintf('%-80s',cryst1);
    output = [output ; cryst1];
end;
if isfield(model.info{snum}.cryst,'scale1'),
    ...
    cryst2 = sprintf('SCALE1    %10.6f%10.6f%10.6f     %10.5f',...
        origx(1),origx(2),origx(3),origx(4));
    cryst2 = sprintf('%-80s',cryst2);
    output = [output ; cryst2];
end;
if isfield(model.info{snum}.cryst,'scale2'),
    origx=model.info{snum}.cryst.scale2;
    cryst2 = sprintf('SCALE2    %10.6f%10.6f%10.6f     %10.5f',...
        origx(1),origx(2),origx(3),origx(4));
    cryst2 = sprintf('%-80s',cryst2);
    output = [output ; cryst2];
end;
if isfield(model.info{snum}.cryst,'scale3'),
    ...
    cryst2 = sprintf('SCALE3    %10.6f%10.6f%10.6f     %10.5f',...
        origx(1),origx(2),origx(3),origx(4));
    cryst2 = sprintf('%-80s',cryst2);
    output = [output ; cryst2];
end;
...
function [output,numCoord,numTer,atom_table] = wr_atom(output,model,chemistry,...
    modnum,snum,cnumbers,cleaned,numCoord,numTer)
...
for mnum=modvec,
    ...
    if models>1,
        MODEL = sprintf('MODEL     %4i',mnum);
        MODEL = sprintf('%-80s',MODEL);
        output = [output ; MODEL];
    end;
    for cnum=cnumbers,
        cid=model.structures{snum}(cnum).name;
            ...
            if backbone,
                ...
                for anum=1:length(pointers), % loop over atoms
                    ...
                    for lnum=1:loc, % loop over locations
                        ...
                        ATOM = sprintf(...
                            '%s%5i %4s%s%s %s%4s   %8.3f%8.3f%8.3f%6.2f%6.2f          %2s',...
                            tline,...
                            atnum,...
                            atag,...
                            ltag,...
                        ...
                        if sum(model.structures{snum}(cnum).Btensor{mnum}(poi,:))>=1, % write ANISOU record
                            ...
                            ANIS = sprintf(...
                                'ANISOU%5i %4s%s%3s %s%4s %7i%7i%7i%7i%7i%7i      %2s',...
                                atnum,...
                                atag,...
                                ltag,...
                                rtag,...
    ...
    if models>1,
        END = sprintf('%-80s','ENDMDL');
        output = [output ; END];
    end;
end;


function [output,numConect] = wr_connect(output,model,snum,cnumbers,atom_table,numConect)
        ...
        for k=1:conny,
            if atom_table(k,2)==cnum,
                ...
                while poi<length(abonds),
                    ...
                    if flush,
                        CONNECT = sprintf(fid,'%s',tline);
                        
                        CONNECT = sprintf('%-80s',CONNECT);
...
function output = wr_master(output, numRemark,numHet,numHelix,numSheet,...
    numSite,numXform,numCoord,numTer,numConect,numSeq)

% write MASTER record
master = sprintf('MASTER    %5i%5i%5i%5i%5i%5i%5i%5i%5i%5i%5i%5i',...
    numRemark,...
    0,...
    numHet,...
    numHelix,...
...
master = sprintf('%-80s',master);
END    = sprintf('%-80s','END');

output  = [output ; master ; END];
