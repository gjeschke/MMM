function varargout = hierarchy_window(varargin)
% HIERARCHY_WINDOW M-file for hierarchy_window.fig
%      HIERARCHY_WINDOW, by itself, creates a new HIERARCHY_WINDOW or raises the existing
%      singleton*.
%
%      H = HIERARCHY_WINDOW returns the handle to a new HIERARCHY_WINDOW or the handle to
%      the existing singleton*.
%
%      HIERARCHY_WINDOW('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HIERARCHY_WINDOW.M with the given input arguments.
%
%      HIERARCHY_WINDOW('Property','Value',...) creates a new HIERARCHY_WINDOW or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before hierarchy_window_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to hierarchy_window_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @hierarchy_window_OpeningFcn, ...
                   'gui_OutputFcn',  @hierarchy_window_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before hierarchy_window is made visible.
function hierarchy_window_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to hierarchy_window (see VARARGIN)

% Choose default command line output for hierarchy_window
...
hMain.hierarchy_window=hObject;
...
% UIWAIT makes hierarchy_window wait for user response (see UIRESUME)
% uiwait(handles.hierarchy_window);


% --- Outputs from this function are returned to the command line.
function varargout = hierarchy_window_OutputFcn(hObject, eventdata, handles) 
...
set(handles.hierarchy_window,'Pointer','watch');
drawnow
...
if handles.snum ~= snum,
    handles.snum = snum;
    handles.page = 1;
    % set_up_pages(handles,lseq,minres,maxres);
end;
...
res_offset = (handles.page-1) * res_per_page;

for k=1:cpl/10+1,
    text((k-1)*10*dw+x0,0,'|','Color',[0.25,0.25,0.75],'FontName','FixedWidth');
end;
for k=1:lneeded,
    text(0,k*dh,sprintf('%i',cpl*(k-1)+1+res_offset),'Color',[0.25,0.25,0.75],'FontName','FixedWidth');
end;
for k=1:lneeded,
    text((cpl+1.5)*dw+x0,k*dh,sprintf('%i',cpl*k+res_offset),'Color',[0.25,0.25,0.75],'FontName','FixedWidth');
end;
for k=1:cpl/10+1,
...
max_res = res_offset + res_per_page;
% Loops
if isfield(model.structures{snum}(cnum),'loop_defs'),
    ...
    for k=1:loopnum, 
        ...
        if hrange(1) < res_offset, hrange(1) = res_offset; end;
        if hrange(2) > max_res, hrange(2) = max_res; end;
...
if isfield(model,'selections') && model.selections>0,
    ...
    if ~isempty(hMain.hierarchy_selected),
        for k=1:length(hMain.hierarchy_selected),
            if hMain.hierarchy_selected(k) ~= 0,
                set(hMain.hierarchy_selected(k),'Color',[0.75,0,0]);
            end;
        end;
        if hMain.hierarchy_selected(end) ~= 0
            set(hMain.hierarchy_selected(end),'Color',[1,0,0]);
        end;
    end;
end;
 
set(handles.hierarchy_window,'Pointer','arrow');


% --- Executes when user attempts to close hierarchy_window.
function hierarchy_window_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to hierarchy_window (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
delete(handles.hierarchy_window);
...
function hierarchy_window_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to hierarchy_window (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in pushbutton_seq_page_minus.
function pushbutton_seq_page_minus_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_seq_page_minus (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
function edit_seq_page_Callback(hObject, eventdata, handles)
% hObject    handle to edit_seq_page (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_seq_page as text
%        str2double(get(hObject,'String')) returns contents of edit_seq_page as a double

[v,handles]=edit_update_MMM(handles,hObject,1,50,1,'%i',1);
handles.page = v;
handles=sequence_display(handles);
guidata(hObject,handles);

% --- Executes during object creation, after setting all properties.
function edit_seq_page_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_seq_page (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in pushbutton_seq_page_plus.
function pushbutton_seq_page_plus_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_seq_page_plus (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
