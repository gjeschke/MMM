...
% Last Modified by GUIDE v2.5 17-Jan-2011 17:00:12
...
hMain.dynamic_rotamers=get(handles.checkbox_dynamic,'Value');
residue_pattern(handles);
...
% --- Executes on button press in checkbox_dynamic.
function checkbox_dynamic_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox_dynamic (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox_dynamic

if get(hObject,'Value'),
    ButtonName = questdlg('This will take 5-10 times longer. Do you really want to do this?', 'Computation time intensive feature', 'No', 'Yes', 'No');
    if ~strcmpi(ButtonName,'Yes'),
        set(hObject,'Value',0);
    end;
end;
