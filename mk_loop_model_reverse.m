function [coor,errcode,restrain,p_model,k] = mk_loop_model_reverse(sequence, anchorC, anchorCn, prot_xyz, restrain, Rama_res, rescodes, min_prob,n_restraints)
% function [coor,errcode,restrain,p_model,k] = mk_loop_model_reverse(sequence, anchorC, anchorCn, prot_xyz, restrain, Rama_res, rescodes, min_prob,n_restraints)
...
%           angles fi,psi)
% rescodes  MMM-internal residue codes for the sequence
...
clash_threshold = 2.0; % 2.8 (was 2.5) minimum distance between atoms in non-consecutive residues
clash_threshold_lp = 2.0; % 2.8 (was 2.5) minimum distance between loop and protein residues (except terminal residues)
            ...
            switch restrain(k).r_beacon(kr).type
                case 'Gaussian'
                    restrain(k).r_beacon(kr).p = ...
                        exp(-((r-restrain(k).r_beacon(kr).par1)/restrain(k).r_beacon(kr).par2)^2);
                ...
                otherwise
                    error('MMM:mk_loop_model_reverse:unknownRestraintType','Restraint type %s not known',restrain(k).r_beacon(kr).type);
            end;
        end;
            ...
            switch restrain(k).r_intern(kr).type
                case 'Gaussian'
                    restrain(k).r_intern(kr).p = ...
                        exp(-((r-restrain(k).r_intern(kr).par1)/restrain(k).r_intern(kr).par2)^2);
            ...
            switch restrain(k).oligomer.type
                case 'Gaussian'
                    restrain(k).oligomer(kr).p = ...
                        exp(-((r-restrain(k).oligomer(kr).par1)/restrain(k).oligomer(kr).par2)^2);
            ...
            switch restrain(k).depth.type
                case 'Gaussian'
                    restrain(k).depth(kr).p = ...
                        exp(-((z-restrain(k).depth(kr).par1)/restrain(k).depth(kr).par2)^2);
