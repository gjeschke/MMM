function [coor,errcode,restrain,p_model] = mk_loop_model(sequence, anchorN, anchorC, anchorNp, anchorCn, prot_coor, restrain, Rama_res, res_defs, min_prob)
...
%           angles fi,psi)
% res_defs  residue definitions from global MMM variable residue_defs
...
for k = 1:ngap+2,
    rescodes(k) = strfind(res_defs.single_letter_code,sequence(k));
end;
            ...
            switch restrain(k).depth.type
                ...
                case 'bounds'
                    if z >= restrain(k).depth(kr).par1 && ...
                            z <= restrain(k).depth.par2, % bounds violated
...
%         repair_dihedrals_fun(backbone);
        ...
        backbone = backbone_f;
        fprintf(1,'Backbone was fixed. Testing constraints.\n');
...
[phi,psi,omega] = dihedrals(poi,backbone);
