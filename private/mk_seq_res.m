function mk_seq_res(fid,sequence)

codes3 = ':GLY:PRO:ALA:VAL:LEU:ILE:MET:CYS:PHE:TYR:TRP:HIS:LYS:ARG:GLN:ASN:GLU:ASP:SER:THR:';
codes1 = 'GPAVLIMCFYWHKRQNEDST';

seqlen = length(sequence);
nl = 1;
chain = 'A';
line = sprintf('SEQRES%4i%2s%5i ',nl,chain,seqlen);
it = 0;
for k = 1:seqlen,
    it = it + 1;
    code = strfind(codes1,sequence(k));
    tag = id2tag(code,codes3);
    line = sprintf('%s %s',line,tag);
    if it >= 13,
        fprintf(fid,'%s\n',line);
        it = 0;
        nl = nl + 1;
