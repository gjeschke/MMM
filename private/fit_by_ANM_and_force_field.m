function [snum_out,fom,rmsd,network,dxmat,DEER,best,diagnostics] = fit_by_ANM_and_force_field(snum_in,network0,DEER,direct,options,test_mode,correspondence,target,ensemble,model,ENM_param)
% function [snum_out,fom,rmsd,network,dxmat,DEER,best,diagnostics] = fit_by_ANM_and_force_field(network0,DEER,direct,maxbas,test_mode,correspondence,target,ensemble)
%
...
add_msg_board('ANM/force field combined transition fit.');

% Control flags for debugging and tests

ff_update=true; % update C_alpha network from constrained force-field 
                 % minimization result, should be true for production runs
ff_accept=false; % accept force field steps that improve constraint rmds,
                 % should be true for production runs
                 
ff_minimize_constraints=true; % if true, constrained force-field 
                              % minimization is performed after ENM fit 
ff_minimize=true; % if true, unconstrained force-field minimization is
                   % performed after ENM fit and possibly constrained
                   % force-field minimization

restraint_force=100; % force constant [kcal/(mol Å)] for restraining C_alpha
                     % positions in intermediate minimization, Tinker
                     % default is 100
                     
force_field='oplsua.prm';
full_force_field=which(force_field);

tolerance=0.1;

maxs=length(model.structures);
newtag=sprintf('tf%i',maxs+1);
delete([general.tmp_files 'rcscwrl4_out.*']);
[snum_out,infile]=repacked_copy(snum_in,newtag);

cmd=sprintf('pdbxyz %s %s',infile,full_force_field);

dospath=which('pdbxyz.exe');
if isempty(dospath),
    message.error=2;
    message.text='Tinker software not found on Matlab path.';
    add_msg_board('This feature requires Tinker''s pdbxyz from the Ponder lab');
...
my_dir=pwd;
cd(modpath);
add_msg_board('Now calling Tinker');

[s, w] = dos(cmd);
runtime=toc;
add_msg_board(sprintf('Tinker pdbxyz was running %5.1f s',runtime));
if s~=0,
    rem=w;
    while ~isempty(rem),
...
[~,basname] = fileparts(infile);
full_bas=fullfile(general.tmp_files,basname);

options.solvation='STILL';
options.restrain='all_CA';
options.restraint_force=restraint_force;

Tinker_correspondence=assign_and_key(network0,full_bas,snum_out,options);

cmd=sprintf('minimize %s %s %6.3f',full_bas,full_force_field,tolerance);

dospath=which('minimize.exe');
if isempty(dospath),
    message.error=2;
    message.text='Tinker software not found on Matlab path.';
    add_msg_board('This feature requires Tinker''s minimize from the Ponder lab');
    add_msg_board('ERROR: Tinker could not be found on the Matlab path.');
    add_msg_board('Please check whether Tinker is installed and the path set.');
    % add_msg_board('(see also help browser)');
    % webcall(entry,'-helpbrowser');
...
[s, w] = dos(cmd);
runtime=toc;
add_msg_board(sprintf('Tinker minimize was running %5.1f s',runtime));
if s~=0,
    rem=w;
    while ~isempty(rem),
        [token,rem]=strtok(rem,char(10));
...
update_from_Tinker(full_bas,snum_out,Tinker_correspondence);

% energy = get_Tinker_minimize(w);
[~,components] = get_Tinker_energy(full_bas,full_force_field);
energy=components.unrestrained;

[net_grad,gradients] = get_Tinker_gradient(full_bas,full_force_field,Tinker_correspondence);
...
ff_energy=zeros(1,ENM_param.cycles+1);
ff_energy(1)=energy;
...
tol_found=false;
tic;
fid_ff=fopen(fullfile(general.tmp_files,'force_field.log'),'wt');
fprintf(fid_ff,'Force field protocol for %s\n\n',full_force_field);
fprintf(fid_ff,'Initial energy: %12.2f kcal/mol\n',energy);

while (no_stop || no_sat) && runtime<maxtime,
    if no_stop>0,
        no_stop=no_stop-1;
    else
        no_stop=0;
    ...
    if ~isempty(net_grad),
        [fom_ff,fulfillment,DEER3,sc_ff]=transition_step_ff(net_grad,gradients,network0,DEER,direct,model);
        fprintf(fid_ff,'Iteration %i: ENM fom is %6.2f Å, ff fom is %6.2f Å\n',numit+1,fom,fom_ff);
        if fom_ff<fom0,
            fprintf(fid_ff,'Successful energy gradient step.\n');
            if ff_accept,
                dx=-sc_ff*net_grad;
                network=network0+dx;
                fom=fom_ff;
            ...
            fprintf(fid_ff,'Energy gradient downhill step increases constraint r.m.s.d.\n');
        end;
        report_fulfillment(fid_ff,fulfillment);
    else
        fprintf(fid_ff,'Ill-defined force-field gradient in iteration step %i.\n',numit+1);
    end;
    fprintf(fid_ff,'\n');
    bas=3*numit;
    ...
    fom_vec(numit)=fom;
    scaling(numit)=sc;
    update_Tinker_from_MMM(full_bas,network,snum_out,Tinker_correspondence);
    mk_Tinker_key(network,full_bas,options,Tinker_correspondence);
    [network1,net_grad,gradients,energy] = get_Tinker_move(full_bas,full_force_field,tolerance,Tinker_correspondence,snum_out);
    if ~isempty(energy),
        ff_energy(numit+1)=energy;
        if numit>0,
            fprintf(fid_ff,'Force field energy in step %i: %12.2f kcal/mol\n',numit,energy);
        end;
        if ff_update,
            network=network1;
        end;
    else
        ff_energy(numit+1)=ff_energy(numit);
        fprintf(fid_ff,'Force field energy in step %i is ill-defined.\n',numit);
    end;
...
if mr>0,
    for k=1:mr, 
        vec=DEER(k).xyz2-DEER(k).xyz1;
        rp=norm(vec);
        fom=fom+weights(k)*(10*DEER(k).r-rp)^2/(mr+md);    
    end;
end;
if md>0,
    for k=1:md, 
...
if test_mode,
    ...
    coverage=(diagnostics.rmsd0-rmsd_struct)/diagnostics.rmsd0;
    fprintf(1,'Coverage after ENM fitting: %6.3f.\n',coverage);
end;

if ff_minimize_constraints,
    options.solvation='Still';
    mk_Tinker_key_constrained(full_bas,network,DEER,direct,options);
    network1 = get_Tinker_move(full_bas,full_force_field,tolerance,Tinker_correspondence,snum_out);
    if ~isempty(network1), 
        network = network1; 
    end;
    if test_mode,
        rmsd_struct=rmsd_superimpose(target(correspondence(2,:),:),network(correspondence(1,:),:));
        coverage=(diagnostics.rmsd0-rmsd_struct)/diagnostics.rmsd0;
        fprintf(1,'Coverage after constrained force-field fitting: %6.3f.\n',coverage);
    end;
end;

if ff_minimize,
    options.solvation='Still';
    mk_Tinker_key_constrained(full_bas,network,[],[],options);
    network1 = get_Tinker_move(full_bas,full_force_field,tolerance,Tinker_correspondence,snum_out);
    if ~isempty(network1), network = network1; end;
    if test_mode,
        rmsd_struct=rmsd_superimpose(target(correspondence(2,:),:),network(correspondence(1,:),:));
        coverage=(diagnostics.rmsd0-rmsd_struct)/diagnostics.rmsd0;
        fprintf(1,'Coverage after unconstrained force-field fitting: %6.3f.\n',coverage);
    end;
end;
...
function [network,net_grad,gradients,energy] = get_Tinker_move(full_bas,full_force_field,tolerance,Tinker_correspondence,snum_out)
...
network=update_from_Tinker(full_bas,snum_out,Tinker_correspondence);
...
function [fom,fulfillment,DEER2,sc]=transition_step_ff(net_grad,gradients,network,DEER,direct,model)
% computes one transition step
...
fulfillment=zeros(md+mr,2);
sq_dir=0;
if md>0,
    for k=1:md, % set up unit vector between restraint residue pairs
        vec=network(direct(k,2),:)-network(direct(k,1),:);
        rp=norm(vec);
        diff=10*direct(k,4)-rp;
        fulfillment(k,1)=diff;
        sq_dir=sq_dir+diff^2;    
    end;
end;
...
if mr>0,
    for k=1:mr,
        ...
        rp=norm(vec);
        diff=10*DEER(k).r-rp;
        fulfillment(md+k,1)=diff;
        sq_deer=sq_deer+diff^2;    
    end;
end;
...
if md>0,
    for k=1:md, 
        ...
        rp=norm(vec);
        diff=10*direct(k,4)-rp;
        fulfillment(k,2)=diff;
        fom=fom+diff^2/(mr+md);    
    end;
end;
if mr>0,
    for k=1:mr, 
        vec=DEER2(k).xyz2-DEER2(k).xyz1;
        rp=norm(vec);
        diff=10*DEER2(k).r-rp;
        fulfillment(k+md,2)=diff;
        fom=fom+diff^2/(mr+md);    
    end;
end;
fom=sqrt(fom);

function report_fulfillment(fid_ff,fulfillment)

[mc,nc]=size(fulfillment);
improved=0;
for k=1:mc,
    fprintf(fid_ff,'Constraint %i deviation before step: %6.2f Å; after step: %6.2f Å\n',k,fulfillment(k,1),fulfillment(k,2));
    if abs(fulfillment(k,2))<=abs(fulfillment(k,1)),
        improved=improved+1;
    end;
end;
fprintf(fid_ff,'\nConstraint r.m.s.d. improves for %i out of %i constraints.\n',improved,mc);

function energy = get_Tinker_energy_update(full_bas,full_force_field,gradients)
