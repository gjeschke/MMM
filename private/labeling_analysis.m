...
if (Options.HtmlOutputLevel>0)
  ...
  fw('<head>\n');
  fw('<title>MMM spin labeling analysis</title>\n');
  fw('<style type="text/css">\n%s</style>',cssStyle);
  fw('</head>\n');
  fw('<body>\n');
  fw('<h1>MMM spin labeling site scan</h1>\n');
...
if Options.DistanceOutput
  ...
  if (hMain.site_scan_intra>0)
    for kc = 1:nChains
      ...
      fw('<A HREF="#intra%i_DEER">DEER</A> | ',kc);
      fw('<A HREF="#intra%i_CW">CW</A> | ',kc);
      fw('<A HREF="#intra%i_DEERlong">long (DEER)</A> | ',kc);
  ...
  if (hMain.site_scan_inter>0) && (nChains>1)
    ...
    for kc1 = 1:nChains-1
      for kc2 = kc1+1:nChains
        ...
        fw('<A HREF="#inter%ito%i_DEER">DEER</A> | ',kc1,kc2);
        fw('<A HREF="#inter%ito%i_CW">CW</A> | ',kc1,kc2);
        fw('<A HREF="#inter%ito%i_DEERlong">DEER long</A> | ',kc1,kc2);
  ...
  if (hMain.site_scan_homooligomer)
    for kc=1:nChains
      ...
      fw('; <A HREF="#oligo%i_DEER">DEER</A> | ',kc);
      fw('; <A HREF="#oligo%i_CW">CW</A> | ',kc);
      fw('; <A HREF="#oligo%i_DEERlong">DEER long</A> | ',kc);
...
for kc = 1:nChains
  ...
  if (Options.HtmlOutputLevel>0)
    ...
    fw('<tr><td colspan="2">Residue<td>label</td><td>location</td><td>NO rmsd</td><td>rotamers</td><td>partition function</td></tr>\n');
...
anchor{4} = '';
anchor{5} = '';
categoryDescr{6}='Distance probably too long for useful measurement (over 7.0 nm)';
categoryDescr{1}='At least one site probably too tight for labeling';
categoryDescr{2}='Distance too short for proper measurement (less than 0.8 nm)';
categoryDescr{3}='Most favorable for CW EPR measurements (0.8 nm ... 1.8 nm)';
categoryDescr{4}='Most favorable for DEER measurements (1.8 nm ... 4.5 nm)';
categoryDescr{5}='DEER measurement possible, but rather imprecise (4.5 nm ... 7.0 nm)';
categoryStr = {'tight','short','CW','DEER','DEERlong','long'};
category(r_mean<0.8) = 2;
category(r_mean>=0.8) = 3;
category(r_mean>=1.8) = 4;
category(r_mean>=4.5) = 5;
category(r_mean>=7.0) = 6;
