function [fom,rmsd,network,dxmat,DEER] = fit_by_ANM_4(network0,DEER,direct,test_mode)
% function [fom,rmsd,network,dxmat,DEER] = fit_by_ANM_4(network0,DEER,direct,test_mode)
%
...
tic;
while no_stop || (~satflag && runtime<maxtime && rmsd>fit_limit && l2d<0.5 && (rmsd>tol_dist || l2d<0.01)),
    if no_stop>0,
        no_stop=no_stop-1;
    else
        no_stop=0;
...
[mi,bestit]=min(abs(change-rmsd_change));

figure(7);
clf;
plot(xx,rmsd_DEER,'k');
title('Distance rmsd vs. structural change');
hold on
plot(xx,change,'r');
plot([bestit,bestit],[0,max([max(rmsd_DEER),max(change)])],'g:');
...
fprintf(1,'Extracting structure at iteration %i.\n',bestit);
rmsd=rmsd_DEER(bestit);
network=networks{bestit};

if ~isempty(DEER),
    DEER=update_labels(DEER,network0,network);
end;
