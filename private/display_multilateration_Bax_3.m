...
hifi = false; % if true, high-fidelity models with >= 0.8 probability
...
switch resnum
    ...
    case 169
        r_intradimer = 16.0; % ### hard-coded constraint ###
        sigr_intradimer = 2.0; % ### hard-coded constraint ###
%         r_intradimer = 30.0; % ### hard-coded constraint ###
%         sigr_intradimer = 5.0; % ### hard-coded constraint ###
    case 193
...
for k=1:m,
    ...
    for kx = 1:nx,
        for ky = 1:ny,
            for kz = 1:nz,
                coor = [x(kx),y(ky),z(kz)];
                rdimer = 2*sqrt(coor(1)^2+coor(2)^2); % intradimer distance is twice the distance of the site from the symmetry axis (z)
...
while structures < num_models && runtime<maxtime,
    runtime = toc;
    trials = trials + 1;
    [coor,label_coorc,errcode,clashes,nofit] = mk_H78_construct_Bax(pcoor, r163);
    if errcode,
        noloop = noloop + 1;
...
save Bax_H78_sh163_sh169_models descr all_loopcoor
