function display_multilateration_Bax_0
...
[~,SG126] = get_object('[BAXC](A)126.SG','coor');

% Clash test

% xc = linspace(0,20,201);
% tcx = zeros(size(xc));
% for k = 1:length(xc),
%     h6scoor = h6coor;
%     h6scoor(:,1) = h6scoor(:,1) + xc(k)*ones(h6m,1);
%     tcx(k) = check_clash(h6scoor,h6coor);
% end;
% figure(1); clf;
% plot(xc,tcx,'b');
% 
% yc = linspace(0,20,201);
% tcy = zeros(size(yc));
% for k = 1:length(yc),
%     h6scoor = h6coor;
%     h6scoor(:,2) = h6scoor(:,2) + yc(k)*ones(h6m,1);
%     tcy(k) = check_clash(h6scoor,h6coor);
% end;
% figure(2); clf;
% plot(yc,tcy,'g');
% 
% zc = linspace(0,20,201);
% tcz = zeros(size(zc));
% for k = 1:length(zc),
%     h6scoor = h6coor;
%     h6scoor(:,3) = h6scoor(:,3) + zc(k)*ones(h6m,1);
%     tcz(k) = check_clash(h6scoor,h6coor);
% end;
% figure(3); clf;
% plot(zc,tcz,'r');
% 
...
% [pm,~] = size(pcoor);
% 
% xc = linspace(0,80,801);
% tcx = zeros(size(xc));
% for k = 1:length(xc),
%     pscoor = pcoor;
%     pscoor(:,1) = pscoor(:,1) + xc(k)*ones(pm,1);
%     tcx(k) = check_clash(pscoor,pcoor);
% end;
% figure(4); clf;
% plot(xc,tcx,'b');
% 
% yc = linspace(0,80,801);
% tcy = zeros(size(yc));
% for k = 1:length(yc),
%     pscoor = pcoor;
%     pscoor(:,2) = pscoor(:,2) + yc(k)*ones(pm,1);
%     tcy(k) = check_clash(pscoor,pcoor);
% end;
% figure(5); clf;
% plot(yc,tcy,'g');
% 
% zc = linspace(0,80,801);
% tcz = zeros(size(zc));
% for k = 1:length(zc),
%     pscoor = pcoor;
%     pscoor(:,3) = pscoor(:,3) + zc(k)*ones(pm,1);
%     tcz(k) = check_clash(pscoor,pcoor);
% end;
% figure(6); clf;
% plot(zc,tcz,'r');
% 
...
[~,coorCA]=get_object('[HEL6]130.CA','coor');
res130CA=coorCA;
...
[~,coorN] = get_object('[HEL6]130.N','coor');
[~,coorCA] = get_object('[HEL6]130.CA','coor');
[~,coorC] = get_object('[HEL6]130.C','coor');
[~,coorO] = get_object('[HEL6]130.O','coor');
...
anchorCn_0 = [coorN;coorCA;coorC;coorO];
...
while structures < num_models && runtime<maxtime,
    ...
    if p<level, % insufficient probability at this grid point
        ...
        MCtry = MCtry + 1;
        ccoor = [xg(index),yg(index),zg(index)];
        shift1 = ccoor - res149t; % bring res 149 N-O midpoint to current grid point
        shift2 = -ccoor;
        r130 = res130CA + shift1 + shift2;
        eulang = [2*pi,pi/2,2*pi].*rand(1,3); % random Euler angles
        rotmat = affine('Euler',eulang);
        r130r = affine_trafo_point(r130,rotmat) - shift2;
        dist = norm(r130r - res126CA); % CA-CA distance 126 - 130
        if dist<5.5 || dist>15, % ### hardcoded loop constraint ###
            misorient = misorient + 1;
        else
            shift4 = repmat(shift1 + shift2,4,1);
            anchorC = anchorC_0 + shift4;
            anchorCn = anchorCn_0 + shift4;
            for k=1:4,
                anchorC(k,:) = affine_trafo_point(anchorC(k,:),rotmat) - shift2;
            end;
            for k=1:4,
                anchorCn(k,:) = affine_trafo_point(anchorCn(k,:),rotmat) - shift2;
            end;
            errcode = 1;
            ltrials = 100;
            kt = 0;
            nonclosed = 0;
            nonRam = 0;
            while errcode && kt < ltrials,
                kt = kt + 1;
                [loopcoor,errcode] = loop_models_short(sequence, anchorN, anchorC, anchorNp, anchorCn, 100);
                if errcode == 1, nonclosed = nonclosed + 1; end;
                if errcode == 3, nonRam = nonRam + 1; end;
            end;
            fprintf(1,'(%i of %i) Trial: %i: %i%% non-closed loops, %i%% Ramachandran disallowed\n',structures,num_models,MCtry,100*nonclosed/ltrials,100*nonRam/ltrials);
            if errcode,
                noloop = noloop + 1;
            else
                shift = repmat(shift1 + shift2,h6m,1);
                h6s = h6coor + shift;
                h6c = h6s;
                for k=1:h6m,
                    h6s(k,:) = affine_trafo_point(h6s(k,:),rotmat) - shift2;
                    h6c(k,:) = affine_trafo_point(h6s(k,:),C2_rotmat);
                end;
                clash = check_clash(h6s,h6c);
                if clash,
                    clashes = clashes + 1;
                else
                    ...
                    if pclash,
                        ...
                        c130(structures,:) = r130r;
                        c149(structures,:) = ccoor;
                        descr(structures).c130 = r130r;
                        descr(structures).l149 = ccoor;
                        descr(structures).eulang = eulang;
                        descr(structures).dist = dist;
                        descr(structures).probability = p;
                        descr(structures).d126 = norm(SG126-ccoor);
...
fprintf(1,'Out of %i trials %i were rejected by residue 149 location probability. %5.2f%% were accepted.\n',trials,too_low,100*(1-too_low/trials)); 
fprintf(1,'Out of %i trials %i were rejected by residue helix 6 misorientation. %5.2f%% were accepted.\n',trials-too_low,misorient,100*(1-misorient/(trials-too_low))); 
fprintf(1,'Out of %i trials %i were rejected by loop modeling failure. %5.2f%% were accepted.\n',trials-too_low-misorient,noloop,100*(1-noloop/(trials-too_low-misorient))); 
fprintf(1,'Out of %i trials %i were rejected by helix 6/helix 6'' clash. %5.2f%% were accepted.\n',trials-too_low-misorient,clashes,100*(1-clashes/(trials-too_low-misorient-noloop))); 
fprintf(1,'Out of %i trials %i were rejected by helix 6/core clash. %5.2f%% were accepted.\n',trials-too_low-misorient-clashes,pclashes,100*(1-pclashes/(trials-too_low-misorient-noloop-clashes))); 
...
save Bax_H6_models descr all_h6coor
