function [message,snum,stag,models]=rd_pdb_ensemble(ensemble,idCode,pname)
% function [message,snum,models]=rd_pdb_ensemble(ensemble,idCode,pname)
%
% Reads several models in PDB format into a single structure by
...
% all models must be analogous, such as models created in a single Modeller
% run
%
% ensemble  list of all models to be read, struct with at least a field
%           .name   file name of model
%           the following fields are used, if they exist 
%           .molpdf Modeller objective function
%           .DOPE   DOPE score
%           .GA341  GA341 score
% idCode    tag for the structure
...
for k=1:length(ensemble),
    ...
    if isempty(info) || isempty(cstructure) % PDB read was not successful
        ...
        models=models+1;
        if k==1, % the first model, structure is initialized
            info1=info;
            ...
            if isempty(info.Modeller_obj) && isfield(ensemble,'molpdf'),
                model.info{snum}.Modeller_obj=ensemble(k).molpdf;
            end;
            model.info{snum}.Modeller_sid=info.Modeller_sid;
            if isfield(ensemble,'DOPE'),
                model.info{snum}.Modeller_DOPE=ensemble(k).DOPE;
            end;
            if isfield(ensemble,'GA341'),
                model.info{snum}.Modeller_GA341=ensemble(k).GA341;
            end;
            dssp=[];
            ...
            if ~isempty(info.Modeller_obj),
                model.info{snum}.Modeller_obj=[model.info{snum}.Modeller_obj info.Modeller_obj];
            end;
            if isempty(info.Modeller_obj) && isfield(ensemble,'molpdf'),
                model.info{snum}.Modeller_obj=[model.info{snum}.Modeller_obj ensemble(k).molpdf];
            end;
            model.info{snum}.Modeller_sid=info.Modeller_sid;
            if isfield(ensemble,'DOPE'),
                model.info{snum}.Modeller_DOPE=[model.info{snum}.Modeller_DOPE ensemble(k).DOPE];
            end;
            if isfield(ensemble,'GA341'),
                model.info{snum}.Modeller_GA341=[model.info{snum}.Modeller_GA341 ensemble(k).GA341];
            end;
