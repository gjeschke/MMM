function [G,msg]=G_propensity(indices,zm,thickness,access,propens)
% function [G,msg]=G_propensity(indices,z,thickness,accessibility,propensities)
%
% hydrophobic energy contribution of residues on insertion into a bilayer
...
for k=1:m,
    ...
    if length(cindices)~=4,
        ...
        if ~isempty(xyz),
            coor=mean(xyz,1);
            position=abs(coor(3)-zm)/thickness;
            propensity=0;
