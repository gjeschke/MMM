...
commands=':acs:atompair:attach:beacons:bckg:bilabel:blscan:camup:camrotate:color:colorscheme:compact:copy:delete:density:detach:dihedrals:distance:domain:download:dssp:echo:helix:help:hide:inertiaframe:label:libcomp:libtest:lock:locrmsd:locorder:loop:mass:motion:mushroom:new:ortho:pdbload:persp:plot:radgyr:redo:refrmsd:remodel:repack:replace:report:rmsd:rotamers:SAS:scopy:select:sheet:show:statistics:symmetry:synonym:transparency:undo:undefine:unlock:unselect:view:wrseq:zoom:';
...
function handles = acs(handles,args)
...
command=sprintf('acs %s',strtrim(args));
undo_cmd='noundo';
[handles]=cmd_history(handles,command,undo_cmd);

if isempty(args) || isempty(strtrim(args))
    add_msg_board('Usage: acs data colorscheme');
    add_msg_board('Data is an MMMx output file.');
    return
end

myargs=textscan(args,'%s');
if length(myargs{1}) < 1
    add_msg_board('ERROR: Command requires at least one argument.');
    return
end
...
if length(myargs{1}) > 1
    colorscheme = char(myargs{1}(2));
else
    colorscheme = 'clustertype';
end

data = get_csv(fname,'header.numerical');

[C,columns] = size(data);
initial_col = [0.2369    0.4437    0.9996];
final_col = [0.9608    0.8902    0.1532];
mixed_col = [0.5 0.5 0.5];
neutral_col = [0    0.5    0.5];
if columns > 6
    ccolors = turbo(max(data(:,8))+2);
else
    ccolors = turbo(C+2);
end
% ccolors = lines(max(data(:,8)));
tint = 0.5;
maxpop = max(data(:,2));
...
for c = 1:C
    ...
    grace = data(c,6);
    pop = data(c,2)/maxpop;
    conformer_sphere(coor,grace,col,pop);
end
