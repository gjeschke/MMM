...
% for selected sites in the protein structure (or model), initialize
...
if isempty(indices),
    ...
    if ischar(sel_residues), % nucleotide case
    ...
    pathname = uigetdir(pwd,'Select directory for saving rotamers');
    if isequal(pathname,0)
        msg.error=1;
        msg.text='User cancelled saving of rotamers';
        add_msg_board('User cancelled saving of rotamers');
        return
