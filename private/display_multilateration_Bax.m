function display_multilateration_Bax
...
while structures < num_models && runtime<maxtime,
    runtime = toc;
    [coor,A,acoor,errcode,clash,nofit] = mk_loop_1_Bax(sequence, anchorN, anchorNp, pcoor, loc126);
    if errcode,
        noloop = noloop + 1;
        fprintf(2,'First half loop modeling failure %i. %i clashes, %i non-fitting constraint.\n',errcode,clash,nofit);
        continue;
    end;
    accepted = false;
    maxtrials = 1000;
    MCtry = 0;
    secfail = 0;
    while ~accepted && MCtry < maxtrials,
        trials = trials + 1;
        MCtry = MCtry + 1;
        [coor,errcode] = mk_loop_2_Bax(sequence,coor,A,acoor,pcoor);
        if errcode,
            secfail = secfail + 1;
            % fprintf(2,'Second half loop modeling failure %i.\n',errcode);
            if secfail < 3,
                continue;
            else
                break;
        ...
        [~,~,transmat]=rmsd_superimpose(anchorC,anchorC_0);
...
%         if MCtry >= maxtrials && ~accepted,
%             % fprintf(2,'No valid model after %i second half trials. Generating new first half loop.\n',maxtrials);
%         end;
    end;
end;
...
fprintf(1,'Out of %i trials %i were rejected by residue 149 location probability.\n',trials,outside); 
fprintf(1,'Out of %i trials %i were rejected by helix 6/helix 6'' clash.\n',trials-outside,clashes); 
fprintf(1,'Out of %i trials %i were rejected by helix 6/core clash.\n',trials-outside-clashes,pclashes); 
...
save Bax_H6_models descr all_h6coor all_loopcoor
