function display_multilateration_Bax_5
...
allowed = find(cube1/mm > level); % allowed indices into probability grid
fprintf(1,'There are %i grid points with sufficient label 169 probability of %4.1f%%.\n',length(allowed),100*level);

coor1 = zeros(length(allowed),3);
for k = 1:length(allowed),
    [py,px,pz] = ind2sub(size(cube), allowed(k));
    coor1(k,:) = [x1(px),y1(py),z1(pz)];
end;
...
allowed2 = find(cube/mm > level); % allowed indices into probability grid
fprintf(1,'There are %i grid points with sufficient label 193 probability of %4.1f%%.\n',length(allowed2),100*level);

coor2 = zeros(length(allowed2),3);
for k = 1:length(allowed2),
    [py,px,pz] = ind2sub(size(cube), allowed2(k));
    coor2(k,:) = [x1(px),y1(py),z1(pz)];
end;
...
nr = 800;
histo = zeros(1,nr+1);
rax = linspace(ra,re,nr+1);

for k1 = 1:length(allowed),
    if mod(k1,10)==0,
        fprintf(1,'At %5.1f%%.\n',100*k1/length(allowed));
    end;
    p1 = cube1(allowed(k1));
    for k2 = 1:length(allowed2),
        p2 = cube(allowed(k2));
        r = norm(coor2(k2,:)-coor1(k1,:));
        poi = 1 + round(nr*(r-ra)/rspan);
        if poi <= nr+1 && poi > 0,
            histo(poi) = histo(poi)+p1*p2;
        end;
    end;
end;
