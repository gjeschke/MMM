...
messages{2}=sprintf('Initial constraint r.m.s.d. %5.2f Å',hMain.fit_diagnostics{k}.drmsd0);
messages{3}=sprintf('Final constraint r.m.s.d.   %5.2f Å',hMain.fit_diagnostics{k}.drmsd);
if ~isempty(hMain.fit_diagnostics{k}.rmsd0) && ~isempty(hMain.fit_diagnostics{k}.rmsd0),
    messages{4}=sprintf('Initial r.m.s.d. to target structure %5.2f Å',hMain.fit_diagnostics{k}.rmsd0);
    messages{5}=sprintf('Final r.m.s.d. to target structure   %5.2f Å',hMain.fit_diagnostics{k}.rmsd);
